...
interface ClassicModeStatsModalProps {
...
export default function ClassicModeStatsModal({
  t,
  bestScores,
  setBestScores,
  playClick,
  ...
  // Yellow Theme for Classic
  const gradient = "from-yellow-500/20 to-amber-600/20"
  const border = "border-yellow-500/30"
  const blur = "bg-yellow-500/5"
  const iconColor = "text-yellow-400"
  const shadow = "drop-shadow-[0_0_15px_rgba(250,204,21,0.5)]"
  const subTitle = "text-yellow-200"
  ...
  return (
              ...
              top5.map((stat, index) => (
                <div key={index} className="bg-white/5 p-3 rounded-xl border border-white/5 flex items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <span className={`text-xl font-black italic tabular-nums text-yellow-400`}>
                      {stat.value}
                    </span>
                  </div>
            ...
            recent.map((entry, i) => (
              <div key={i} className="bg-white/5 p-4 rounded-2xl border border-white/5 flex items-center justify-between gap-4">
                ...
                <div className="flex flex-col items-start min-w-[80px]">
                  <span className={`text-2xl font-black italic leading-none tabular-nums text-yellow-400`}>{entry.score}</span>
      ...
      <div className="bg-white/5 p-4 sm:p-6 rounded-[2rem] border border-white/5 mt-4">
        ...
        >
          {confirmReset ? t.confirmDeleteAll : `${t.clearRecordsFor || "Clear records for"} ${t.modeClassic || 'Classic'}`}
          <Trash2 size={16} />
        </button>
      </div>
      </div>
